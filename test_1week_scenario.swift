    ...
    func updateTimelineIfNeeded(birthDate: Date, simulatedCurrentDate: Date) {
        ...
        if didAddSomethingNew {
            self.periods.sort { $0.startDate > $1.startDate } // Yeniden eskiye sıralama korunur
             ...
             print("ℹ️ Değişiklik yok. Her şey güncel.")
        }
    }
}
...
// Çocuk bugün tam 1 haftalık (8 Ocak 2026 = 7 gün sonra)
let installDate = df.date(from: "2026-01-08T12:00:00Z")! 
...
print("\n-----------------------")
print("⏳ 1 Hafta geçti...")
print("📱 SENARYO 2: Ebeveyn 1 hafta sonra uygulamayı tekrar açtı. (Çocuk: 2 haftalık / 15 Ocak 2026)")
let oneWeekLater = df.date(from: "2026-01-15T12:00:00Z")!
